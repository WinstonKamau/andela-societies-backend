...
from api.utils.marshmallow_schemas import (post_activity_schema)
...
class ActivitiesAPI(Resource):
    ...
    def post(self):
        ...
        result, errors = post_activity_schema.load(payload)

        if errors:
            response = jsonify(errors)
            status_code = post_activity_schema.context.get('status_code')
            post_activity_schema.context = {}
